...
func GuessHQSystem(incursion ESI.IncursionResponse, esi ESI.ESIClient) (hqSystem NamedItem) {
	...
	logging.Debugf("Staging %d, should be %d vanguards and %d assaults", incursion.StagingID, vanguards, assaults)
	...
	defer func() {
		if r := recover(); r != nil {
			logging.Errorln("Recovered from panic while guessing HQ system", r)
	...
	logging.Infof("Guessed that HQ system was %s\n", hqSystem.Name)
	return hqSystem
}
